...
보안은 서비스가 제공하는 신뢰입니다. 어떤 데이터를 어떻게 보호할 것인지. 어떤 접근을 제한할 것인지 하는 보안 정책이 없다면 대규모 시스템에서 필연적으로 보안 약점이 그러날 것입니다. 특히 클라우드 환경에서 아키텍처를 설계할 때, 허가되지 않은 접근을 차단할 수 있도록 철저하게 보안운영을 하는것이 중요합니다. 

## ERP , MES시스템에 대해서 알고 있는지  
ERP: Enterprise Resource Planning의 약자로, 기업의 재무 인사 생산 등 핵심 업무 프로세스를 통합 관리하는 시스템 및 소프트웨어를 의미합니다. 반복적인 업무를 줄여 생산성을 높이고, 데이터를 추출하여 유의미한 의사결정에 사용할 수 있어 최근에 많이 도입하고 있습니다.

MES: Manufacturing Execution System 생산현장의 전반을 관리하는 시스템. 실적, 설비 가동, 제품 품질 등 데이터 수집, 분석/모니터링하여 전체 공정을 관리하고 개선


## 서버 장애 발생시 문제 역추적을 하는데 있어 어떤 점을 고려하고, 우선순위는 어떻게 해야하는지

일단 에러가 발생하면, 가장 중요한 것은 서비스 정상화입니다. 먼저, 영향 범위를 파악해야합니다. 에러가 어떤 사람, 어떤 데이터에 영향을 주었는지를 빠르게 파악해야합니다. 그리고 빠른 롤백, 서비스 재시작 등등으로 빠르게 복구해야합니다.

그 다음, 장애 발생 지점을 찾습니다. 서비스가 안정화 되었거나 즉각적인 조치를 못했다면, 장애가 발생한 지점을 찾아야 합니다. CPU 나 메모리 사용량이 초과했는지 아니면 어떤 API에서 문제가 발생했는지 DB 문제인지를 로그를 통해 파악해야 합니다. 또한 MSA 환경에서는 하나의 API요청이 여러 서비스를 거치기도 합니다. 그래서 정확히 어디서 에러가 발생했는지 툴을 사용하여 파악해야 합니다.

그 후, 최종적으로는 재발 방지를 위해 시스템을 개선해야 합니다. 장애 감지가 느렸다면 모니터링을 추가한다던가, CI/CD파이프라인을 개선하는 등 인프라적인 개선이 필요할수도 있습니다.



## 이 직무를 선택한 이유와 관련된 경험.

아키텍처 직무는, 서비스 전체의 구조를 설계하는 업무로 알고 있습니다. 클라우드 혹은 온프레미스 서버에 서비스를 위한 환경을 세팅하고, 자잘하게는 어떻게 트래픽을 처리할지, 비기능적 요구사항을 잘 처리할지 고민하며 프로젝트 전체의 구조를 설계합니다.

저는 그동안 여러 서비스를 만들고 운영하며, 서비스 구조를 잡는 것의 중요성에 대해 배웠습니다. 코드 레벨에서는 클린 아키텍처와 같은 것을 적용하여, 코드 간 의존성이 줄어 유지보수성이 좋아지는 것을 경험했고, 인프라 레벨에서는 CI/CD 파이프라인을 구성하여 개발자가 코드에만 집중할 수 있는 환경을 만들고, 모놀리틱 구조를 갖출지 MSA 구조를 도입할지에 대해 여러번 고민하며 의사결정 능력을 길렀습니다. 

또한 클라우드 환경에서 서비스를 운영하며 IAM 유저를 관리하고, CodePipeline이나 secrets manager와 같은 AWS의 다양한 서비스들을 유기적으로 관리하여 서비스를 구성해 보는 등 아키텍처 적인 역량을 길렀습니다.

이렇게 다양한 경험들로 아키텍처 직무의 중요성을 느꼈고, 또 이런 전체 시스템을 설계하는 일에 흥미를 느껴 아키텍처 직무를 지원하게 되었습니다.
...
리더쉽에 관한 경험이 있습니다. 
제가 현재 컴퓨터 종합 설계라는 대학교 졸업 수업을 수강하고 있습니다. 저희 팀은 5명으로 구성되어있고, 저희 학교의 챗봇 서비스를 개발하고 있습니다. 그런데, 팀원분들이 아무래도 개발 관련 지식이 부족해서 제가 리더를 맡아서 전체적인 개발을 이끌고 있습니다.

먼저, 각각 팀원들이 해본 경험이 있는 것을 기준으로 업무를 분배하고, 전체 프로젝트의 구조를 설계하여 팀원들에게 구조를 설명해주고 있습니다. 또한, 각각 맡고 있는 업무에 대해서 대부분 크롤링, 프론트엔드 이렇게 제가 경험해 본 부분이라, 제가 도움을 줄 수 있는 부분은 주고 또 AI도 적극적으로 활용하여 효율적으로 개발하고 있습니다.

### laas paas saas 에 대해서

서비스 제공자가 어느 범위까지 제공해 주는지에 따라 구분합니다.
laas는 서버와같은 인프라만 빌려줍니다. EC2등이 속합니다.
Paas는 애플리케이션을 배포할 수 있는 플랫폼을 빌려줍니다. 개발자는 단순히 코드만 올리면 배포 등 다른 부분은 고려할 필요가 없습니다.
Saas는 이미 완성된 소프트웨어를 대여해줍니다. 로그인해서 사용하면 되므로, 따로 관리할것이 없습니다.


## DX의 의미
디지털 전환을 의미합니다. 단순히 오프라인 업무를 디지털로 옮기는 것을 넘어, 클라우드, ai, 빅데이터와 같은 디지털 기술을 기업의 핵심 전략으로 삼습니다. 기업의 비즈니스 모델 자체를 근본적으로 혁신하고, 운영 프로세스를 최적화하여 생산성을 높이고 새로운 고객 가치를 창출하게 됩니다.

## LG CNS를 지원한 이유

## 다른 회사에 합격해도 LG CNS를 올것인지?

네 물론입니다. 저는 아키텍처로서 제가 어떠한 환경에서 가장 성장할 수 있을까를 생각했을 때, 그 곳이 LG CNS라고 확신있게 말할 수 있습니다. 국내에서, 금융 공공의 미션 크리티컬한 프로젝트를 다양하게 수행해본 경험을 가지고 있고, 다양한 프로젝트를 경험해보며 아키텍처로서 성장할 수 있는 가장 좋은 환경을 갖추고 있는 곳이라 생각합니다. 저는 서비스 기업보다 SI 회사를 선호하고, SI 회사 중에서는 LG CNS가 최고라고 생각하기 때문에 LG CNS가 저의 1순위 회사입니다.
...
SDS는 자사의 플랫폼을 LG보다 많이 사용한다는 것을 들었다. LG CNS에서 더 다양한 솔류션을 경험할 수 있을것 같았다.

삼성SDS는 자체 솔루션과 제조/물류 쪽에 강점이 있고, LG CNS는 클라우드와 금융쪽에 강점이 있다고 들었습니다. 자체 솔루션을 많이 도입하는 삼성보다는 LG CNS에서 더 다양한 기술을 경험해볼 수 있겠다고 생각했습니다. 그리고 금융도메인에 제가 조금 더 관심이 있습니다. 평소에 엄격한 안정성이 요구되는 금융 시스템의 구조 또한 궁금했는데, LG CNS에서는 이런것도 경험해볼 수 있을것이라 생각했습니다.
...
클라우드의 서비스는 외부 네트워크의 접속이나 그런걸 엄격하게 제한합니다. 네트워크 통신 비용이 많이 들고 오버헤드 또한 증가할 것 같습니다. 그리고 여러 클라우드에 일관되게 보안 설정을 하는 것 또한 어려울 것 갓ㅌ습니다.
## 그레이존이 생길 경우 어떻게 하는지?  

업무의 책임 소재가 불분명한 일. 아키텍트는 개발 인프라 기획 등 여러 팀의 경계에서 일하는 직무이므로, 이런 일이 자주 생길것 같습니다.

일단 가장 먼저 팀원 혹은 리더와 이 안건에 대해 공유하겠습니다. 그리고 해당업무를 파악하여, 책임자를 명확히 정의하겠습니다. 그리고 아무도 나서지 않는다면 제가 주도적으로 나서서 해결하겠습니다.
## K8S 네트워크 관련 질문
ClusterIP는 클러스터 내부에서만 접근할 수 있는 IP를 할당
Nodeport는 특정 포트를 열어, 외부에서 접근할 수 있도록 합니다. 테스트 목적으로 사용하거나, 외부 로드밸런서를 수동으로 구성할 때 사용합니다.
LoadBalancer방식은, 클라우드 제공업체와 통신하여, 외부 로드밸런서를 자동으로 생성합니ㅏㄷ.
...
개발자로서 저는 안정과 혁신이라는 가치를 가장 중요하게 생각합니다. 그리고 LG CNS는 이러한 가치를 모두 갖추고 있는 회사라고 생각하여 지원하였습니다.

먼저, LG CNS는 금융, 공공권의 다양한 미션크리티컬한 프로젝트를 수행해왔습니다. 특히, 백신예약 시스템이 오류났을때, 아키텍처 팀에서 빠르게 문제를 해결해 준 일은 CNS의 안정성을 잘 보여준다고 생각합니다.

그리고 LG CNS는 단순한 SI 회사를 넘어, DX, AM등 시대의 변화에 빠르게 맞춰가며 혁신하고 있습니다. 최근에는 AI의 급격한 발전에 따라, AX전환도 돕고, DAP GEN AI라는 자체 생성형 AI 플랫폼까지 갖추는 등, 혁신성을 보여주고 있습니다.

이러한 환경의 LG CNS에서 다양한 프로젝트를 경험하며 아키텍처 전문가로 성장하고 싶어 지원하였습니다.

# 1분 자기소개

안녕하세요. LG CNS Architecture 직무에 지원하게 된 채민관입니다.

개발자로서 두가지 핵심역량을 기르기 위해 노력해왔습니다.
...
LG CNS에서 원활한 의사소통 역량을 바탕으로 고객의 비즈니스 요구에 맞는 최적의 클라우드 아키텍처를 설계하고 안정적으로 운영하는 전문가로 LG CNS와 함께 성장하겠습니다.


## 왜 서비스 회사가 아닌 SI 업체에 지원했는지

네 저는 아키텍트로서 어떤 경험이 저를 더 성장시킬 수 있을까를 많이 생각해봤습니다. 물론 자체 운영하는 서비스에서 아키텍처를 운영하는 것도 매력적이라고 생각합니다.
하지만 이런 환경에서는 다양한 아키텍처를 경험해보기는 어려울것이라 생각했습니다. 아키텍처로 성장하기 위해서는, 어떠한 아키텍처를 적용할지 의사결정을 위해 다양한 프로젝트를 통해 아키텍처를 경험해보는 것이 더 중요하다 생각했습니다.

## 개발이 아닌 아키텍처 직무에 지원한 이유

여러 서비스를 개발하는 경험을 거치며, 코드 레벨에서 아키텍처 설계가 코드 간 의존성을 줄여 개발 비용이 감소하는 것을 경험했고, 인프라 레벨에서 설계한 아키텍처가 서비스 안정성에 도움을 주고, CI/CD파이프 라인이 개발에 집중할 수 있는 환경을 만들어주는것을 경험했습니다. 이러한 경험들로 코드 개발과 아키텍처 모두에 관심을 가지고 있습니다.

그러나 최근에 AI가 급격히 발달하면서, 직접 코드를 타이핑 하는 일이 많이 줄었습니다.  특히, CI/CD같은 서비스의 인프라 구조, 그리고 코드 아키텍처 레벨에서 코드의 구조를 잘 잡아놓으면, 개발 생산성이 훨씬 증가한다고 생각합니다.
그래서 최근에는 코드 개발보다는 아키텍처 쪽에 조금 더 관심을 두고 있고, 아키텍처 직무에 지원하게 되었습니다.
## AWS 상에서의 보안

가장 먼저, 
IAM 권한을 떠올릴 수 있을것 같습니다. 계정 보안은 가장 중요한 요소입니다. 그래서 최소권한 원칙을 적용하여, 필요한 권한만을 가진 IAM 유저를 생성하고, 이를 활용하여 서비스에 접근해야 합니다. 그리고 계정에 MFA를 꼭 사용해야 합니다.
다음으로는 네트워크 레벨에서의 보안입니다. AWS에서는 VPC를 기본으로 사용합니다. 외부 노출이 불필요한 DB같은 서비스는 프라이빗 서브넷에 배치합니다.
또한 보안 그룹을 사용해서, 특정 포트로의 접근만 허용하는 것이 좋습니다.
그리고 소스코드 내부적으로도, SQL 인젝션이나, CORS은 것들을 적용하여 보안을 강화해야하고, API key와 같은 민감 정보를 외부에 노출되지 않게 해야합니다.

CloudTrail 서비스를 사용해서, 누가 언제 어떤 API를 호출했는지 등등 모든 활동을 기록해야합니다.
...
## '운영 효율성'을 높이는 아키텍처는 구체적으로 어떤 모습일까요?" 

(예: CI/CD 파이프라인, 모니터링/로깅(MSA) 등)
...
## "궁극적으로 '대규모 트래픽을 다루는 시스템'을 설계하고 싶다고 하셨습니다. 현재 수준의 트래픽을 처리하는 시스템과 대규모 트래픽을 처리하는 시스템의 **아키텍처는 근본적으로 무엇이 달라야 할까요?**" 
(이 질문은 이전 대화에서 다룬 Scale-out, DB 확장, 캐시, MQ 등을 답변할 기회입니다.)
